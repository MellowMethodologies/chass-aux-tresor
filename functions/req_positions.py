...
def get_positions(language, x, y, direction, limit, indice):
    url = f'https://api.dofusdb.{language}/treasure-hunt?x={x}&y={y}&direction={direction}&limit={limit}'
    ...
    for location in data:
        for i in location['pois']:
            if (i['name']['fr'] == indice):
                return location['posX'], location['posY']

print(get_positions('fr', 0, 0, 6, 1, 'Grelot'))